...
internal sealed class DispatcherReviewMappings : Profile
{
    public DispatcherReviewMappings()
    {
        CreateMap<DispatcherReview, DispatcherReviewDto>()
            .ForCtorParam(nameof(DispatcherReviewDto.DriverId), cfg => cfg.MapFrom(e => e.CheckPoint.DriverId))
            .ForCtorParam(nameof(DispatcherReviewDto.DriverName), cfg => cfg.MapFrom(e => $"{e.CheckPoint.Driver.FirstName} {e.CheckPoint.Driver.LastName}"))
